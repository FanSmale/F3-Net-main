...
def SaveTrainValidResults(train_loss, val_loss, SavePath, ModelName, font2, font3):
    ...
    ax.set_xlim([1, 10])
    ax.set_xticks([i for i in range(0, thirdstage_epochs+1, 20)])
    ax.set_xticklabels((str(i) for i in range(0, thirdstage_epochs+1, 20)))
...
def SaveTestResults(TotPSNR, TotSSIM, ToMSE, ToMAE, ToUQI, ToLPIPS, ToBMSE, ToBMAE, Prediction, GT, SavePath):
    ...
    print('TotPSNR: {}, TotSSIM: {}, ToMSE: {}, ToMAE: {}, ToUQI: {}, ToLPIPS: {}, ToBMSE: {}, ToBMAE: {}'.format(
        np.mean(TotPSNR), np.mean(TotSSIM), np.mean(ToMSE),
        np.mean(ToMAE), np.mean(ToUQI), np.mean(ToLPIPS),
        np.mean(ToBMSE), np.mean(ToBMAE))
    )

    # Excel 路径
    file_path = os.path.join(SavePath, 'save_result.xlsx')

    # 如果文件存在，读取；否则新建一个 DataFrame
    if os.path.exists(file_path):
        df = pd.read_excel(file_path)
    else:
        df = pd.DataFrame(columns=[
            'ModelName', 'TotPSNR', 'TotSSIM', 'ToMSE',
            'ToMAE', 'ToUQI', 'ToLPIPS', 'ToBMSE', 'ToBMAE'
        ])

    # 当前测试结果
    Test_data = pd.DataFrame([{
        'ModelName': ModelName,   # 注意：ModelName 要在全局定义
        'TotPSNR': np.mean(TotPSNR),
        'TotSSIM': np.mean(TotSSIM),
        'ToMSE': np.mean(ToMSE),
        'ToMAE': np.mean(ToMAE),
    ...
    file_path = os.path.join(SavePath, 'save_result.csv')
    df.to_csv(file_path, index=False)

    # 保存为 .mat
    scipy.io.savemat(os.path.join(SavePath, ModelName + '_TestResults.mat'), data)

    print(f"✅ 测试结果已保存到 {file_path} 和 {ModelName}_TestResults.mat")
